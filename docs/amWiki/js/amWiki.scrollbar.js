...
(function (win, $) {
    ...
    Scroller.prototype._init = function () {
		...
		this.$e.boxY.on('click', function(e){
			if(e.clientY - that._data.barYTop < parseFloat(that.$e.sliderY.css('top'))){
				that.scrollYTo(parseFloat(that.$e.sliderY.css('top')) - that._data.sliderH, 500);
			}else if(e.clientY - that._data.barYTop > parseFloat(that.$e.sliderY.css('top')) + that._data.sliderH){
				that.scrollYTo(parseFloat(that.$e.sliderY.css('top')) + that._data.sliderH, 500);
			}
		});
        this.checkHeightWidth();
        this._bindAction();
		this._data.barYTop = $(".scrollbar-y").offset().top;
    };
    ...
    Scroller.prototype._resize = function () {
        ...
        //计算最大高度
        this._data.maxTop = (this._data.barH - this._data.sliderH) / this._data.barH * this._data.contentH - 133;
		//console.log("barH = " + this._data.barH);
		//console.log("sliderH = " + this._data.sliderH);
		//console.log("maxTop = " + this._data.maxTop);
        //水平方向
        if (this._data.xRolling) {
    ...
    Scroller.prototype._reScroll = function () {
        ...
        var barTop = top1 / (this._data.maxTop) * (this._data.barH - this._data.sliderH);
		...
		if(!barTop) $("#backTop").stop().fadeOut();
		else $("#backTop").stop().fadeIn();
        //水平方向
        if (this._data.xRolling) {
    ...
    /**
     * 垂直滚动到某位置
     * @param {Number} num mstime
     * @public
     */
    Scroller.prototype.scrollYTo = function (num, mstime) {
        var barTop = num;
        barTop = barTop < 0 ? 0 : barTop;
        barTop = barTop + this._data.sliderH > this._data.barH ? this._data.barH - this._data.sliderH : barTop;
		if(!mstime){
			this.$e.sliderY.css('top', barTop);
			this.$e.inner.scrollTop(barTop / (this._data.barH - this._data.sliderH) * this._data.maxTop);
			if(!barTop) $("#backTop").stop().fadeOut();
			else $("#backTop").stop().fadeIn();
			return;
		}
		this.$e.sliderY.animate({"top": barTop}, mstime);
		this.$e.inner.animate({"scrollTop": barTop / (this._data.barH - this._data.sliderH) * this._data.maxTop}, mstime, function(){if(barTop < 1) $("#backTop").stop().fadeOut(); else $("#backTop").stop().fadeIn();});
    };
